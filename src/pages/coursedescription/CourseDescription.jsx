...
const CourseDescription = ({user}) => {
  ...
  const{data:{ order },
    ...
    const options={
      "key": "rzp_test_M0l3wmNfz7nW7r", // Enter the Key ID generated from the Dashboard
    "amount": order.id, // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise
    "currency": "INR",
    "name": "Study-Guru", //your business name
    "description": "Reviving Internal Teacher",
 ...
 <>
 {
  ...
  {course && <div className="course-description">
    ...
    <p>Buy This Course At ₹{course.price}</p>
      {
        user && user.subscription.includes(course._id) ? (
        <button onClick={()=>navigate(`/course/study/${course._id}`)} className="common-btn">Study</button>
        ):(
        <button onClick={checkoutHandler} className="common-btn">Buy</button>
        )
      }
    </div>}
  </>
